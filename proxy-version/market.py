...
def createacc(server,prioritylist,useclient=None,proxy=None,proxytype='HTTP',callback=None,goodstate=None,totalrunning=None,timeout=30):
	...
	try:
		...
		try:
			...
			pass
		totalrunning[0]-=1
def startmarket(server,prioritylist,useclient=None,proxy=None,proxytype='HTTP',callback=None,checksource=None,totalconn=1,timeout=30):
	totalrunning=[0]
	goodstate=[0]
	for i in range(0,totalconn):
		totalrunning[0]+=1
		Thread(target=createacc,args=(server,prioritylist,useclient,proxy,proxytype,callback),kwargs={'goodstate':goodstate,'totalrunning':totalrunning,'timeout':timeout}).start()
	while True:
		if checksource!=None:
