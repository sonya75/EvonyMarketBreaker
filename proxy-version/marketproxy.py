...
def finishdatahandler(value):
	global totalfoodsold,totalwoodsold,totalironsold,totalstonesold,linecount
	if (value[:15]!='SUCCESSFULTRADE'):
		linecount+=1
		if linecount>=100:
			wx.CallAfter(frame1.m_textCtrl3.SetValue,'')
			linecount=0
		wx.CallAfter(frame1.m_textCtrl3.AppendText,(value+"\n"))
		return
	value=value[15:]
	value1=value.split(' ')
	if value1[0]=='food':
		totalfoodsold+=int(value1[1])
		wx.CallAfter(frame1.m_staticText17.SetLabel,("Food Sold : "+format(totalfoodsold,',')))
		return
	if value1[0]=='wood':
		totalwoodsold+=int(value1[1])
		wx.CallAfter(frame1.m_staticText23.SetLabel,("Wood Sold : "+format(totalwoodsold,',')))
		return
	if value1[0]=='stone':
		totalstonesold+=int(value1[1])
		wx.CallAfter(frame1.m_staticText24.SetLabel,("Stone Sold : "+format(totalstonesold,',')))
		return
	if value1[0]=='iron':
		totalironsold+=int(value1[1])
		wx.CallAfter(frame1.m_staticText25.SetLabel,("Iron Sold : "+format(totalironsold,',')))
		return
